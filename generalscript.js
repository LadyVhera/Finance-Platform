...
function countTo(el, target, suffix = "") {
  ...
  function frame(t) {
    ...
    const val = Math.floor(
      start + (target - start) * (0.5 - 0.5 * Math.cos(Math.PI * p))
    );
...
document.getElementById("toggleSignInPwd").addEventListener("click", () => {
  const p = document.getElementById("signInPassword");
  p.type = p.type === "password" ? "text" : "password";
  document.getElementById("toggleSignInPwd").textContent =
...
document.getElementById("signInForm").addEventListener("submit", (e) => {
  ...
  if (!user) {
    alert("Sign in failed (demo). Check email/password.");
    return;
  }
  // demo session
...
document.getElementById("forgotLink").addEventListener("click", (e) => {
  e.preventDefault();
  const email = prompt(
    "Enter your email for password-reset instructions (demo):"
  );
  if (!email) return;
  const found = getUsers().find((u) => u.email === email.trim().toLowerCase());
  if (!found) {
    alert("Email not found (demo).");
    return;
  }
  alert(
    "Demo: a password-reset link would be sent to your email (not implemented)."
  );
});
...
(function () {
  ...
  grid.addEventListener("click", (e) => {
    ...
    lbTag.textContent = (card.dataset.category || "").replace(/^\w/, (c) =>
      c.toUpperCase()
    );
    lb.show();
  });
...
(function () {
  ...
  function openAt(index) {
    ...
    tagEl.textContent = (it.dataset.category || "").replace(/^\w/, (c) =>
      c.toUpperCase()
    );
    bsModal.show();
  }
  ...
  modalEl.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowRight")
      modalEl.querySelector(".lightbox-next")?.click();
  });
})();
...
(function () {
  ...
  links.forEach((a) => {
    a.addEventListener("click", (e) => {
      ...
      if (
        navCollapse &&
        window.getComputedStyle(navCollapseEl).display !== "none"
      ) {
        navCollapse.hide();
      }
    });
...
(function () {
  ...
  document.querySelectorAll('#mainNav .nav-link[href^="#"]').forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      const doScroll = () => {
        const y =
          target.getBoundingClientRect().top + window.pageYOffset - getOffset();
        window.scrollTo({ top: y, behavior: "smooth" });
      };
...
(function () {
  ...
  freshForm.addEventListener("submit", (e) => {
    ...
    if (typeof saveUser === "function") {
      ...
      try {
        const key =
          typeof USERS_KEY !== "undefined" ? USERS_KEY : "tpt_demo_users_v2";
    ...
    localStorage.setItem(
      "tpt_demo_session",
      JSON.stringify({ email, name, role: "member" })
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Hard fallback: force-show everything that AOS might have hidden
